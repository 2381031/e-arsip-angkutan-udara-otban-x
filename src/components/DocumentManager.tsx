            ...
            <form onSubmit={handleUploadSubmit} className="space-y-4">
              ...
              <div>
                ...
                <div className="border border-dashed border-slate-300 dark:border-slate-700 rounded-xl p-4 bg-slate-50 dark:bg-slate-950 text-center hover:bg-slate-100/50 transition">
                  <input
                    ...
                    disabled={uploading}
                  />
                  <label htmlFor="file-upload" className={`cursor-pointer flex flex-col items-center ${uploading ? "pointer-events-none opacity-60" : ""}`}>
                    {formFile ? (
                      <>
                        <FileText className="w-8 h-8 text-emerald-500 mb-2" />
                        <span className="text-xs font-bold text-slate-700 dark:text-slate-200 break-all text-center px-2">
                          {formFile.name}
                        </span>
                        <span className="text-[10px] text-emerald-600 dark:text-emerald-400 mt-1 font-bold font-mono">
                      ...
                      <>
                        ...
                        <span className="text-xs font-bold text-slate-700 dark:text-slate-200">
                          Klik di sini untuk mencari berkas
                        </span>
              ...
              {uploading && (
                <div className="bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800 rounded-xl p-4 space-y-3">
                  <div className="flex items-center gap-3">
                    <span className="w-5 h-5 border-2 border-emerald-600 border-t-transparent rounded-full animate-spin shrink-0"></span>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-bold text-emerald-700 dark:text-emerald-300">
                        {uploadProgress < 100 ? "Mengirim file ke server..." : "Menyimpan ke database..."}
                      </p>
                      <p className="text-[10px] text-emerald-600 dark:text-emerald-400 font-mono mt-0.5">
                        {formFile && (
                          <>
                            {((formFile.size * uploadProgress) / (100 * 1024 * 1024)).toFixed(1)}MB / {(formFile.size / (1024 * 1024)).toFixed(2)}MB
                          </>
                        )}
                      </p>
                    </div>
                    <span className="text-sm font-extrabold text-emerald-700 dark:text-emerald-300 font-mono tabular-nums shrink-0">
                      {uploadProgress}%
                    </span>
                  </div>
              ...
              <div className="pt-4 flex justify-end gap-3 border-t border-slate-100 dark:border-slate-800">
                ...
                <button
                  ...
                  className="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white font-bold rounded-xl text-xs flex items-center gap-1.5 cursor-pointer disabled:opacity-50 min-w-[140px] justify-center transition"
                >
                  {uploading ? (
